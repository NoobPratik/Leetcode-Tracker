...
class SettingView:
    def __init__(self, tabview):
        ...
        discord_var = StringVar(value=tabview.discord_forum_id)
        ...
        self.discord_label = CTkLabel(frame, text='Discord Forum ID: ')
        self.discord_entry = CTkEntry(frame, textvariable=discord_var)
        ...
        self.ide_path_entry.grid(row=0, column=1, padx=(20, 20), pady=(20, 20), sticky="nsew", columnspan=2)
        self.leetcode_session_label.grid(row=1, column=0, padx=(20, 20), pady=(20, 20), sticky="nsew")
        self.leetcode_session_entry.grid(row=1, column=1, padx=(20, 20), pady=(20, 20), sticky="nsew", columnspan=2)
        ...
        self.discord_label.grid(row=2, column=1, sticky="nsew")
        self.discord_entry.grid(row=3, column=1, padx=(20, 20), pady=(0, 20), sticky="nsew")
        self.server_ip_label.grid(row=2, column=2, sticky="nsew")
        self.server_ip_entry.grid(row=3, column=2, padx=(20, 20), pady=(0, 20), sticky="nsew")

        radio_button_1.grid(row=5, column=0, padx=30, pady=10, sticky="w")
        radio_button_2.grid(row=5, column=1, padx=30, pady=10, sticky="w")
        self.apply_button.grid(row=6, column=0, columnspan=3, padx=(20, 20), pady=(20, 20), sticky="s")

        self.discord_var = discord_var
        self.server_ip_var = server_ip_var

    def apply(self):
        self.tabview.app.execute(
            '''UPDATE user_settings 
               SET ide_path=?, leetcode_session=?, ide_option=?, user_name=?, 
                   discord_forum_id=?, server_ip_address=? 
               WHERE id = 1''',
        ...
        self.tabview.discord_forum_id = self.discord_entry.get()
